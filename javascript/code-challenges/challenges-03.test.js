...
const typeNum = (arr) => {
  const numsOnly = arr.filter((element) => typeof element === "number");
  return numsOnly;
};
...
const containsAnd = (arr) => {
  //if a word has letter[i] = a, letter[i+1] = n, letter[i+2] = d
  // const hasAnd = arr.filter(word => word[])?
  let hasAnd = arr.filter((word) => word.includes("and"));
  return hasAnd;
};
...
const oddValues = (arr) => {
  let ans = arr.filter((number) => number % 2);
  return ans;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  return arr.filter((nums) => {
    if (!forbiddenValues.includes(nums)) return nums;
  });
};
