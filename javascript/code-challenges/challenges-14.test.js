...
const screenForNames = (arr) => {
  // Solution code here...
  let validator = /^[MD][rs]s?\.\s[a-zA-Z]+/g;
  return arr.filter((screened) => screened.match(validator));
};
...
const toTitleCase = (arr) => {
  // take the first letter of each word and capitalize it, then add the rest of the word, MacGyver will be unchanged
...
let biggerThanLuke = (arr) => {
  // get luke's mass, filter out any objects whose mass is not greater than lukes, return their names and join with " - "
  let luke = arr.find((char) => char.name === "Luke Skywalker");
  return arr
    .filter((sithRebel) => Number(sithRebel.mass) > Number(luke.mass))
    .map((sithRebel) => sithRebel.name)
    .join(" - ");
};
...
const sortBy = (property, arr) => {
  //think sort low to high, by number or by letter.
};
...
------------------------------------------------------------------------------------------------ */
const isSecure = (url) => {
  // must begin with https://, the /'s will need to be escaped
  const validator = /^https:\/\//;
