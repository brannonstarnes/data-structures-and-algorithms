...
const count = (target, input) => {
  ...
  input
    .map(array => array
      .forEach(number => number === target ? targetCount++ : false));
  return targetCount;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  let ans = input
    .map(elements => elements
      .filter(elements => typeof elements === 'number' && elements % 5 === 0))
    .map(elements => elements
...
let findShortest = (data) => {
  let smallest = data
    .reduce((heightOne, heightTwo) => {
      Number(heightOne.height) > Number(heightTwo.height) ? heightOne = heightTwo : heightOne = heightOne;
      return heightOne;
    });
