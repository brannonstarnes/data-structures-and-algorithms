...
const replaceZeros = (string) => {
  let validator = /0/g; //find anything with a 0 in it
  return string.replace(validator, 'zero'); //replace all 0's with 'zero'
};
...
const validatePin = (pin) => {
  let validator = /\b\d{4}\b/g; //this means there should only be 4 numberical 'd'igits in the expression
  return validator.test(pin); //test against the rules of the validator: will either return true or false
};
...
const validateWord = (word) => {
  let validator = /\b\D{5,10}\b/g; // D is anything NOT a numerical digit. must use comma in the range to say min and max length
...
const validateEmail = (email) => {
  let validator = /^[a-zA-Z0-9]+\.[a-zA-Z0-9]*@[a-z]+\.[a-z]{3}$/;
